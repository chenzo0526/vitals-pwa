...
type NextLabRecommendations = {
  essential: NextLabItem[]
  valuable_additions: NextLabItem[]
  skip_for_now: SkipLabItem[]
}
...
type Interpretation = {
  ...
  next_lab_recommendations?: NextLabRecommendations
...
export default function BloodworkPanelDetailPage() {
  ...
  const [copiedLabs, setCopiedLabs] = useState(false)
        ...
        <CardContent className="p-4 space-y-4">
          ...
          {interpretation && (
            <div className="space-y-4">
              ...
              {/* Next Lab Recommendations — premium "order next time" section with rationale + copy */}
              {interpretation.next_lab_recommendations
                ? <NextLabsSection
                    recs={interpretation.next_lab_recommendations}
                    copied={copiedLabs}
                    onCopy={async () => {
                      const recs = interpretation.next_lab_recommendations!
                      const lines: string[] = []
                      lines.push(`Lab order — generated by Vitals on ${new Date().toLocaleDateString()}`)
                      lines.push(`Based on panel: ${panel.panel_name || 'Untitled'} (${panel.drawn_on || 'date unknown'})`)
                      lines.push('')
                      lines.push('=== ESSENTIAL ===')
                      recs.essential.forEach(l => lines.push(`• ${l.marker} — ${l.cadence}`))
                      if (recs.valuable_additions.length > 0) {
                        lines.push('')
                        lines.push('=== VALUABLE ADDITIONS ===')
                        recs.valuable_additions.forEach(l => lines.push(`• ${l.marker} — ${l.cadence}`))
                      }
                      try {
                        await navigator.clipboard.writeText(lines.join('\n'))
                        setCopiedLabs(true)
                        setTimeout(() => setCopiedLabs(false), 2500)
...
function NextLabsSection({
  recs, copied, onCopy,
}: {
  recs: NextLabRecommendations
  copied: boolean
  onCopy: () => void
}) {
  return (
    <div className="rounded-xl border border-blue-400/30 bg-gradient-to-br from-blue-500/[0.06] to-transparent p-3 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs uppercase tracking-wider font-bold text-blue-300 flex items-center gap-1.5">
          <Beaker size={12} /> Order at your next draw
        </p>
        <button
          onClick={onCopy}
          className="text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-md bg-blue-400/15 border border-blue-400/40 text-blue-200 hover:bg-blue-400/25 flex items-center gap-1"
        >
          {copied ? <><ClipboardCheck size={11} /> Copied!</> : <><Copy size={11} /> Copy list</>}
        </button>
      </div>
      <p className="text-[10px] text-white/40 leading-relaxed">
        Paste this into your LabCorp / Quest order request or hand it to your practitioner. Saves $50-150 in unnecessary tests.
      </p>

      {recs.essential.length > 0 && (
        <div>
          <p className="text-[10px] uppercase tracking-wider text-rose-300 font-bold mb-1.5">Essential</p>
          <div className="space-y-1.5">
            {recs.essential.map((l, i) => (
              <div key={i} className="rounded-lg border border-rose-400/20 bg-rose-500/[0.06] p-2">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-bold text-rose-100">{l.marker}</p>
                  <p className="text-[10px] text-rose-200/70 font-mono whitespace-nowrap">{l.cadence}</p>
                </div>
                <p className="text-[11px] text-white/70 mt-1 leading-relaxed">{l.why}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {recs.valuable_additions.length > 0 && (
        <div>
          <p className="text-[10px] uppercase tracking-wider text-amber-300 font-bold mb-1.5">Valuable additions</p>
          <div className="space-y-1.5">
            {recs.valuable_additions.map((l, i) => (
              <div key={i} className="rounded-lg border border-amber-400/20 bg-amber-500/[0.06] p-2">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-bold text-amber-100">{l.marker}</p>
                  <p className="text-[10px] text-amber-200/70 font-mono whitespace-nowrap">{l.cadence}</p>
                </div>
                <p className="text-[11px] text-white/70 mt-1 leading-relaxed">{l.why}</p>
              </div>
            ))}
      ...
      {recs.skip_for_now.length > 0 && (
        <div>
          <p className="text-[10px] uppercase tracking-wider text-white/40 font-bold mb-1.5">Skip this time</p>
          <div className="space-y-1.5">
            {recs.skip_for_now.map((l, i) => (
              <div key={i} className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
                <p className="text-xs font-medium text-white/60 line-through">{l.marker}</p>
                <p className="text-[11px] text-white/50 mt-0.5 leading-relaxed">{l.reason}</p>
              </div>
            ))}
          </div>
        </div>
